<?php
	...
	$required_fields = array("supervisor_name", "supervisor_phone",	"employee_first_name",
		"employee_last_name", "employee_unit", "delete", "form_type");
	$fields = array("supervisor_name"=>80, "supervisor_phone"=>20, "employee_first_name"=>45,
		"employee_last_name"=>45, "catcard"=>20, "employee_unit"=>60, "delete"=>1, "form_type"=>45);
	...
	if(!$stmt){
		error_log("submit_remove.php:53: Error in preparing statement: ".$conn->error);
		exit(0);
	}
	$stmt_emp = Array();
	$stmt_params = Array();
	...
	foreach($fields as $key=>$max_length){
		if($key=="delete"){
			$stmt_emp[$key] = !empty($_POST[$key]) ? 1 : 0;
		}else{
			$stmt_emp[$key] = substr($_POST[$key], 0, $max_length);
		}
	}
	if(!$stmt->execute()){
		error_log("submit_remove.php:69: Failed to insert record: ".$conn->error);
		exit(0);
	}
	
	$to = "su-web@email.arizona.edu";
    // $to = "su-tech-serv@email.arizona.edu";
	$subject = "Remove Building Access - ".$_POST["employee_first_name"]." ".$_POST["employee_last_name"];
