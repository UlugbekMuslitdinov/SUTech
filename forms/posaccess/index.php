...
?>
<h1>Point of Sale Access Request Form</h1>
<?php
...
function validateAndSubmit(){
	...
	for(var i in employees){
		...
		if(employees[i].emplid.value != ""){
			if(/[^\d\s]+/.test(employees[i].emplid.value)){
				employees[i].emplid.className+=" required_error";
				errors.push("Employee #"+String(i*1+1)+": The user's Employee ID IS a number, not like the NetID");
			}
		}
		if (employees[i].access.checkset["Cashier Access"].checked || employees[i].access.checkset["Lead Cashier Access"].checked) {
			if(employees[i].catcard.value==""){
				employees[i].catcard.className+=" required_error";
				errors.push("Employee #"+String(i*1+1)+": You must provide a Catcard number to request cashier access.");
			}
			if(employees[i].register.value!=""){
				if(!employees[i].register.value.match(/^[0-9]+$/)){
					employees[i].register.className+=" required_error";
					errors.push("Employee #"+String(i*1+1)+": The register pin may only contain numbers.");
				}else if(employees[i].register.value.length != 4){
					employees[i].register.className+=" required_error";
					errors.push("Employee #"+String(i*1+1)+": The register pin must be exactly 4-digits.");
				}
			}
		}
...
<div id="employee_info_proto" class="employee_info_multi">
	...
	<div style="width: 723px;">
		...
		<div class="form_field">
			...
			<div>
				<input id="employee_id_proto" type="text" maxlength="30" class="field_text" value="<?php if (isset($_SESSION["formdata"]["employee_id"])) {echo $_SESSION["formdata"]["employee_id"];} ?>" />
			</div>
			<span class="info-text">
				(This <b>IS</b> a number, not the NetID)
			</span>
		</div>
		<div class="form_field proto-checkset" style="width: 100%;" id="access_proto">
			...
			<label class="access_option showhide-trigger-register">
				<input value="Cashier Access" type="checkbox" />
				Cashier Access
			</label>
				<label class="access_option showhide-trigger-register">
				<input value="Lead Cashier Access" type="checkbox" />
				Lead Cashier Access
			</label>
			<label class="access_option">
				<input value="Sequoia Web Reporting" type="checkbox" />
				Sequoia Web Reporting
			</label>
		</div>
		<div class="form_field showhide-target-register">
			<div class="field_title">
				Catcard#: <span class="field_required">*</span>
			</div>
			<div>
				<input id="catcard_proto" type="text" maxlength="16" class="field_text" />
			</div>
		</div>
		<div class="form_field showhide-target-register">
			<div class="field_title">
			...
			<div>
				<input id="register_pin_proto" type="password" maxlength="20" class="field_text" />
			</div>
			<div class="info-text" style="color: #111; padding-top: 2px;">Leave blank for no change.</div>
		</div>
		<div class="form_field proto-checkset showhide-target-updates" style="width: 100%;" id="updates_proto">
			...
			<label class="access_option">
				<input value="Profit Center Location Change" type="checkbox" />
				Profit Center Location Change
			</label>
			<label class="access_option">
				<input value="Remove Profit Center Location" type="checkbox" />
				Remove Profit Center Location
			</label>
			<label class="access_option">
				<input value="Cashier Access Level" type="checkbox" />
				Cashier Access Level
			</label>
			<label class="access_option">
				<input value="Catcard # Update" type="checkbox" />
				Catcard # Update
			</label>
			<label class="access_option">
				<input value="Pin # Change" type="checkbox" />
				Pin # Change
			</label>
		</div>
