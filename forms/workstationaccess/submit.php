...
//	require_once("sidebar.php");
	...
	$addtl_required_fields = array("request_type", "employee_first_name",
		...
		"employee_unit", "employee_netid", "workstation_tag", "access");
	$addtl_fields = array("request_type"=>20, "employee_position"=>20, "employee_first_name"=>45,
		...
		"employee_unit"=>60, "employee_netid"=>60, "workstation_tag"=>140, "access"=>0,
		"foodpro_location"=>45, "updates"=>0, "comments"=>250);
	...
	$conn = select_db("sucs");
	$query  = "INSERT INTO workstation_access_requests (";
	...
	if($count_employees > 1){
		$subject = "Workstation Systems Access Request - ".$count_employees." Employees";
	}else{
		$subject = "Workstation Systems Access Request - ".$_SESSION["formdata"]["employee_multi"][0]["employee_type"]." - ".$_SESSION["formdata"]["employee_multi"][0]["employee_first_name"]." ".$_SESSION["formdata"]["employee_multi"][0]["employee_last_name"];
	}
	...
	foreach($_SESSION["formdata"]["employee_multi"] as $num=>$emp){
		...
		Workstation Tag: <b>'.$emp['workstation_tag'].'</b><br />
    ...
    if ($email) {
		...
		if($count_employees > 1){
			...
			$subject = "Workstation Systems Access Request - ".$_SESSION["formdata"]["employee_multi"][0]["employee_first_name"]." ".$_SESSION["formdata"]["employee_multi"][0]["employee_last_name"];
		}
		$headers = 'From: TECH Web Mailer <su-tech@email.arizona.edu>' . "\r\n" .
		'Reply-To: TECH Web Mailer <no-reply@tech.union.arizona.edu>' . "\r\n" .
		'X-Mailer: PHP/' . phpversion();
...
?>
<h1>Workstation Access Request Form</h1><br />
<?php
if ($email) {
